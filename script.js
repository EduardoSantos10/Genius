...
const colorParts = document.querySelectorAll(".color");
...
const generateRandomPath = async () => {
  randomColor.push(getRandomColor(colorObj));
  score = randomColor.length;
  isPathGenerating = true;
  await showPath(randomColor);
};
...
const endGame = () => {
  resultEl.innerHTML = `<span> Your Score : </span> $
    {score}`;
...
const handleColorClick = async (e) => {
  ...
  //
  if (e.target.classList.contains(randomColor[clickCount])) {
    e.target.style.backgroundColor = colorObj[randomColor[clickCount]].new;
    await delay(500);
    e.target.style.backgroundColor = colorObj[randomColor[clickCount]].current;
    clickCount++;
